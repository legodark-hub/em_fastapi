...
class BaseService:
    ...
    async def get_by_query_one_or_none(self, *args: Any, **kwargs: Any) -> Any | None:
        return await self.uow.__dict__[self.base_repository].get_by_query_one_or_none(*args, **kwargs)
    
    @transaction_mode
    async def get_by_query_all(self, *args: Any, **kwargs: Any) -> Sequence[Any]:
        return await self.uow.__dict__[self.base_repository].get_by_query_all(*args, **kwargs)
